...
class GPUPowerFlow(object):
    ...
    def power_flow_gpu(
        ...
        # start = perf_counter()
        self.gpu_solver(S_ca, K_ca, V0_ca, W_ca, m_int, p_int, tolerance_int, iterations_int, convergence_int)
...
def generate_network(nodes, child=3, plot_graph=False, load_factor=2, line_factor=3):
    ...
    nodes_properties_ = pd.DataFrame(np.tile([[Tb, Pct, Ict, Zct]], (nodes, 1)), columns=["Tb", "Pct", "Ict", "Zct"])
    nodes_properties = pd.concat([power, nodes_properties_], axis=1)
    nodes_properties = nodes_properties.astype({"Tb": int, "PD": float, "QD": float, "Pct": int, "Ict": int, "Zct": int})
    ...
    lines_properties = pd.DataFrame(np.tile([[R, X, 0, 1, 1]], (LINES, 1)), columns=["R", "X", "B", "STATUS", "TAP"])
...
def create_pandapower_net(network_info: dict, branch_info: pd.DataFrame = None, bus_info: pd.DataFrame = None):
    ...
    # Add buses
    bus_dict = {}
    for bus_name in bus_info["NODES"]:
    ...
    # Lines
    active_branches = branch_info[branch_info["STATUS"].astype(float) != 0].reset_index(drop=True)
    for i, (_idx, (from_bus, to_bus, res, x_react, b_susceptance)) in enumerate(active_branches[["FROM", "TO", "R", "X", "B"]].iterrows()):
    ...
    # Loads:
    for node in bus_info["NODES"]:
        pp.create_load(net, bus=bus_dict[node], p_mw=0.02, q_mvar=0.0, name="Load")
...
def test_create_pandapower_net(network_info=None):
    ...
    if network_info is None:
        data_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data_sources", "network_data", "node_34"))
        network_info = {
