...
class AgentBase:
    ...
    def __init__(self, net_dims: [int], state_dim: int, action_dim: int, gpu_id: int = 0, args: Config = Config()):
        ...
        self.cri_optimizer = torch.optim.AdamW(self.cri.parameters(), self.learning_rate) if cri_class else self.act_optimizer
    ...
    def explore_vec_env(self, env, horizon_len: int, if_random: bool = False) -> Tuple[Tensor, ...]:
        ...
        get_action = self.act.get_action
        for t in range(horizon_len):
            action = torch.rand(self.num_envs, self.action_dim) * 2 - 1.0 if if_random else get_action(state).detach()
            states[t] = state  # state.shape == (num_envs, state_dim)

            state, reward, done, _ = step_env(env, action)  # next_state
            actions[t] = action
            rewards[t] = reward
            dones[t] = done
...
class ActorSAC(nn.Module):
    ...
    def get_action_logprob(self, state: Tensor) -> [Tensor, Tensor]:
        ...
        log_prob -= (np.log(2.0) - a_noise - self.soft_plus(-2.0 * a_noise)) * 2.0
...
class AgentSAC(AgentBase):
    ...
    def get_obj_critic_raw(self, buffer, batch_size: int) -> Tuple[Tensor, Tensor]:
        ...
        obj_critic = self.criterion(q1, q_labels) + self.criterion(q2, q_labels) / 2.0  # twin critics
        return obj_critic, states
    ...
    def get_obj_critic_per(self, buffer: ReplayBuffer, batch_size: int) -> Tuple[Tensor, Tensor]:
        ...
        td_errors = (self.criterion(q1, q_labels) + self.criterion(q2, q_labels)) / 2.0
        obj_critic = (td_errors * is_weights).mean()
