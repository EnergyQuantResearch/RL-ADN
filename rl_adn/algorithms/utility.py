...
class SumTree:
    ...
    def update_ids(self, data_ids: Tensor, prob: Tensor = 10.0):  # 10 is max_prob
        """
...
def get_episode_return(env, act, device):
    ...
    for i in range(env.episode_length):
        ...
        for node_index in env.battery_nodes:
            violation = min(0, 0.05 - abs(1.0 - post_control_voltage[node_index]))
            if violation < 0:
                violation_time += 1
        violation_value += violation

        reward_for_power += info["reward_breakdown"]["economic"]
        reward_for_good_action += 0
        reward_for_penalty += info["reward_breakdown"]["voltage_penalty"]
